<?
	...
	$r2 = mysql_query("SELECT * from municipio WHERE id ='".$_GET['id']."'  
	AND id_dist = '".$_SESSION['user_id']."' AND id_prov ='".$_GET['idprov']."' LIMIT 1");
...
<div class="modal-header">
    ...
    <h4 class="modal-title" id="myModalLabel"><?=$acc_text?> Minicipio de la provincia <?=Provincia($_GET['idmuni'])?></h4>
</div>
            ...
            <div class="form-group ">
                ...
                <? if(!$_GET['id']){?>
                   ...
                   <input name="fecha" type="hidden" id="fecha" value="<?=date('Y-m-d G:i:s')?>" />
                    ...
                    <input name="id_prov" type="hidden" id="id_prov" value="<?=$_GET['idprov']?>" />
                <? } ?> 
       </div>     
            <div class="modal-footer">
               <button type="button" class="btn btn-danger" data-dismiss="modal">Cerrar</button>
               <button name="acep" type="button" id="acep" class="btn btn-success" onClick="CargarAjax2_form('Admin/Sist.Administrador/Provincia/Municipio/List/listado.php?idprov=<?=$_GET['idprov']?>&pagina=1','form_edit_perso','cargaajax');"><?=$acc_text?></button>
