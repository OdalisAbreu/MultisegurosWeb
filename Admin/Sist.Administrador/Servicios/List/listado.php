<?
	...
	if($acc1=='registrar'){
		...
		$query=mysql_query("UPDATE seguro_tarifas SET id_serv =CONCAT(id_serv,'".$_POST['id']."-') WHERE id IN (".$idtarif.")  AND id_serv NOT LIKE '%".$_POST['id']."-%'");
		
		//echo "UPDATE seguro_tarifas SET id_serv =CONCAT(id_serv,'-".$_POST['id']."-') WHERE id IN (".$idtarif.")  AND id_serv NOT LIKE '%-".$_POST['id']."-%'<br>";
		
		
		
		$query=mysql_query("UPDATE seguro_tarifas SET id_serv = REPLACE(id_serv,'".$_POST['id']."-','') WHERE id NOT IN (".$idtarif.")  AND id_serv LIKE '%".$_POST['id']."-%'");
...
<div class="row" >
                <div class="col-lg-10" style="margin-top:-35px;">
                    <h3 class="page-header">Listados de Servicios adicionales </h3>
                </div>
                
            <div class="col-lg-2" style=" margin-top:10px;">
            <a onClick="CargarAjax2('Admin/Sist.Administrador/Servicios/List/listado.php','','GET','cargaajax');" class="btn btn-info">
             <i class="fa fa-list fa-lg"></i></a>
            <a  onClick="CargarAjax_win('Admin/Sist.Administrador/Servicios/Edit/editar-registar.php?accion=registrar','','GET','cargaajax');"  class="btn btn-primary">
 ...
 $query=mysql_query("SELECT * FROM servicios WHERE id_dist ='".$_SESSION['user_id']."' order by id ASC");
...
<tr>
    ...
    <td>
      ...
      <!--editar -->
            <a href="javascript:" onclick="CargarAjax_win('Admin/Sist.Administrador/Servicios/Edit/editar-registar.php?id=<?=$row['id']; ?>','','GET','cargaajax');" data-title="Editar"  class="btn btn-info">
    ...
    if ($row['activo']=='si'){ ?>
		 ...
		 <!--desactivar -->
            <a href="javascript:Elim();" onclick="if(confirm('Deshabilitar \n &iquest; Esta seguro de seguir ?')){ CargarAjax2('Admin/Sist.Administrador/Servicios/List/listado.php?action=desactivar&id=<?=$row['id']; ?>','','GET','cargaajax'); }" data-title="Desactivar"  class="btn btn-danger">
	...
	<?   }else{ ?>
         ...
         <!--activar -->
            <a href="javascript:Elim();" onclick="if(confirm('Deshabilitar \n &iquest; Esta seguro de seguir ?')){ CargarAjax2('Admin/Sist.Administrador/Servicios/List/listado.php?action=activar&id=<?=$row['id']; ?>','','GET','cargaajax'); }"  data-title="Activar"  class="btn btn-primary">
