...
ajax_email.open("GET","Admin/Sist.Administrador/Suplidores/Cuentas/Edit/verifica_email.php?q="+comprueba_mail+"&id="+id, false);
		ajax_email.send();
...
function validarEmail(email) {
	...
	function Validar_Suplid() {
		...
		CargarAjax2_form('Admin/Sist.Administrador/Suplidores/Cuentas/List/listado.php','form_edit_perso','cargaajax');
...
<div class="modal-header">
    ...
    <h4 class="modal-title" id="myModalLabel"><?=$acc_text?>
     Cuenta Suplidor <? if($_GET['id_suplidor']){ echo "de ".NombreProgS($_GET['id_suplidor']); }?></h4>
</div>
    ...
    <div class="row">
               ...
               <? if(!$_GET['id_suplidor']) {?>   
                 ...
                 <label class="strong">Suplidor</label>
                     <div class="form-group ">
                      <select name="id_suplid" id="id_suplid" style="display: inline; font-size:16px;" class="form-control">
                        <? ///  SELECCION DEL TIPO .....................................
    ...
    while ($cat2 = mysql_fetch_array($rescat2)) {
			...
			if($row['id_suplid'] == $id){
               ...
               <? if(!$_GET['id']){?>
                ...
                <input name="funcion_id" type="hidden" id="funcion_id" value="36" />
                
                <? if($_GET['id_suplidor']){?>
                <input name="id_suplid" type="hidden" id="id_suplid" value="<?=$_GET['id_suplidor']?>" />
                <? } ?>
       </div>     
