...
?>
    ...
    if ($_GET['nuevo'] == '1') { ?>
        ...
        <script>
            $(document).ready(function() {
                CargarAjax2('../ws2/TareasProg/GenerarReporteAseguradoraPdf_Global_VIANewVersion.php?fecha1=<?= trim($_GET['fecha1']) ?>&fecha2=<?= trim($_GET['fecha2']) ?>&ruta_id=<?= $_GET['ruta_id'] ?>&user_id=<?= trim($_GET['user_id']) ?>', '', 'GET', 'manejo');
    ...
    ?>
        ...
        <script>
            $(document).ready(function() {
                CargarAjax2('../ws2/TareasProg/GenerarReporteAseguradoraPdf_Global_VIA.php?fecha1=<?= trim($_GET['fecha1']) ?>&fecha2=<?= trim($_GET['fecha2']) ?>&ruta_id=<?= $_GET['ruta_id'] ?>&user_id=<?= trim($_GET['user_id']) ?>', '', 'GET', 'manejo');
