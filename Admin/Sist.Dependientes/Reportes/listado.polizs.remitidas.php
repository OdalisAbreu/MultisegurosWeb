					...
					<tr>
						<td>
							...
							<? if ($_GET['consul'] == '1') { ?>
								...
								<a href="Admin/Sist.Dependientes/Reportes/Export/listado.polizs.remitidas.php?fecha1=<?= $fecha1; ?>&fecha2=<?= $fecha2; ?>&idpers=<?= $_GET['idpers'] ?>&ruta_id=<?= $_GET['ruta_id'] ?>&consul=1" class="btn btn-primary btn-icon glyphicons download_alt hidden-print" id="descargar">Export</a>
								
								<? if ($_SESSION['dist_id'] == '89') { ?>
									<a href="Admin/Sist.Dependientes/Reportes/Export/listadoCsqPolizasRemitidas.php?fecha1=<?= $fecha1; ?>&fecha2=<?= $fecha2; ?>&idpers=<?= $_GET['idpers'] ?>&ruta_id=<?= $_GET['ruta_id'] ?>&consul=1" class="btn btn-info btn-icon glyphicons download_alt hidden-print" id="descargar">Export CSQ</a>
								<? } ?>
				...
				<script type="text/javascript">
					$('#bt_buscar').click(
						function() {
							...
							CargarAjax2('Admin/Sist.Dependientes/Reportes/listado.polizs.remitidas.php?fecha1=' + fecha1 + '&fecha2=' + fecha2 + '&ruta_id=' + ruta_id + '&consul=1', '', 'GET', 'cargaajax');
						...
						<tbody>
							...
							if ($_GET['consul'] == '1') {
								...
								$query = mysql_query("SELECT 
											...
											WHERE
												rever.id_trans IS NULL AND trans.monto !='' AND trans.user_id ='" . $_SESSION['dist_id'] . "'  $wFecha $resulAgen order by id DESC");

								while ($row = mysql_fetch_array($query)) {

									$Agent = explode("-", $row['x_id']);
									$Agencia = explode("/", Agencia($Agent[0], $_SESSION['dist_id']));
	...
	echo  "</pre>";*/
								...
								foreach ($array as $key => $val) {
									...
									foreach ($val as $key2 => $row) {
										...
										$Agencia2 = explode("/", Agencia($Agent2[0], $_SESSION['dist_id']));
