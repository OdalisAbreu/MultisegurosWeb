...
<h2>Transferencia distribuidas de los bancos del <? echo $fecha1;?></h2>
 ...
 <script type="text/javascript">
	...
	var url = 'Admin/Sist.PersonalRecargas/List/Listado_recargas_trans_new.php?fecha1='+fecha1+'&fecha2='+fecha2+'&impr=1&consul=1';
	...
	CargarAjax2('Admin/Sist.PersonalRecargas/List/Listado_recargas_trans_new.php?fecha1='+fecha1+'&fecha2='+fecha2+'&consul=1','','GET','cargaajax');
...
<?
	...
	$wFecha2 = "AND fecha_inic >= '$fDesde 00:00:00' AND fecha_inic < '$fHasta 23:59:59'";
	//para el listado nuevo
...
if($_GET['consul']==1){	
  ...
  $query=mysql_query(" 
  SELECT id,fecha,id_pers,monto, cuenta_banco,client_afect,comentario
  FROM recarga_balance_cuenta
  WHERE $wIndexId2 realizado_por = '".$_SESSION['user_id']."' AND autorizada_por='6' $wFecha 
  ...
  ?>
   ...
   <tr>
	...
	<?=NombreBanco($row['cuenta_banco']); ?>
    
    </a>
    
    </td>
      <td><?="<font color='#006699'>Contado</font>"; $monto_contado += $row['monto'];?> </td>
  </tr>
  <? if($row['comentario'] !==''){ ?>
  <tr>
  ...
  if($_GET['consul']==1){	
	  ...
	  SELECT *  FROM credito2 
	  WHERE $wIndexId tipo = 'credito' AND fecha >= '$fDesde 00:00:00' AND fecha < '$fHasta 23:59:59' AND realizado_por2 = '".$_SESSION['user_id']."' ORDER BY id_banco DESC
	  ");
  ...
  if($_GET['consul']==1){	
	  ...
	  SELECT nota,id,fecha,id_pers,monto,id_banco FROM credito2 
	  WHERE tipo = 'abono_credito' $wFecha AND realizado_por2 = '".$_SESSION['user_id']."'
